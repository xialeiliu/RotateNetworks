...
# flowers imshow convenience function
def flowers_imshow(img,im_size):
    plt.imshow(img.reshape([im_size,im_size,3]))
    plt.axis('off')

# load Oxford-102 Flowers dataset
def load_flowers(path, inp_size):
    ...
    for m in xrange(len(img_names)):
        # read the image
        data = cv2.imread(path+"jpg/"+img_names[m])
...
# return a new flowers dataset
def disjoint_flowers(flowers,nums):
    pos_trn = []
    for i in range(len(nums)):
        tmp = np.where(np.asarray(flowers[3]) == nums[i])[0]
        pos_trn = np.hstack((pos_trn,tmp))
        pos_trn  = np.asarray(pos_trn).astype(int)
        np.random.shuffle(pos_trn)
    pos_tst = []
    for i in range(len(nums)):
        tmp = np.where(np.asarray(flowers[5]) == nums[i])[0]
        pos_tst = np.hstack((pos_tst,tmp))
        pos_tst  = np.asarray(pos_tst).astype(int)
        np.random.shuffle(pos_tst)
    
    trn_img = itemgetter(*pos_trn)(flowers[0])
    val_img = flowers[1]
    tst_img = itemgetter(*pos_tst)(flowers[2])
    trn_lbl = itemgetter(*pos_trn)(flowers[3])
    val_lbl = flowers[4]
    tst_lbl = itemgetter(*pos_tst)(flowers[5])
